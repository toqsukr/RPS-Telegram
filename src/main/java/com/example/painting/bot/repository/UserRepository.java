...
import com.example.painting.bot.exceptions.DbException;
import com.example.painting.utils.UserMapper;
...
public class UserRepository implements IUserRepository {
    ...
    public void insert(User entity) throws DbException {
        try {
            // В параметры запроса все поля сущности кроме идентификатора, т.к. он serial и
            // генерируется автоматом
            var result = template.update(SQL_INSERT,
                    entity.getName(),
                    entity.getDescription());
            if (result != 1)
                log.trace("UserRepository.update() with {} rows inserted", entity);
    ...
    public void delete(User entity) throws DbException {
        try {
            ...
            if (result != 1)
                log.trace("UserRepository.delete() with {} rows inserted", entity);
